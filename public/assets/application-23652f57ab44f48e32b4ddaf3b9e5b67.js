...
(function() {
  jQuery(function() {
    var coordinate, coordinates, graticule, height, i, len, path, projection, results, svg, width, xy;
    width = 960;
    height = 960;
    projection = d3.geo.mercator().scale((width + 1) / 2 / Math.PI).translate([width / 2, height / 2]).precision(.1);
    path = d3.geo.path().projection(projection);
    graticule = d3.geo.graticule();
    svg = d3.select('body').append('svg').attr('width', width).attr('height', height);
    svg.append('path').datum(graticule).attr('class', 'graticule').attr('d', path);
    d3.json('us', function(error, world) {
      if (error) {
        throw error;
      }
    ...
    for (i = 0, len = coordinates.length; i < len; i++) {
      ...
      results.push(svg.append("circle").attr({
        cx: xy[0],
        cy: xy[1],
        r: 2,
        fill: '#e2b506'
